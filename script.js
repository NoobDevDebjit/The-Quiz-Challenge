...
function submitPayment() {
    ...
    // Check if UPI ID has been used before (check in Firebase)
    checkUpiIdInUse(upiId).then(isInUse => {
        if (isInUse) {
        ...
        // Add to Firebase
        savePendingPaymentToFirebase(payment);
...
// Function to check if UPI ID is already in use
async function checkUpiIdInUse(upiId) {
    const snapshot = await database.ref('usedUpiIds').once('value');
    const usedUpiIds = snapshot.val() || [];
    return usedUpiIds.includes(upiId);
}

// Function to save pending payment to Firebase
function savePendingPaymentToFirebase(payment) {
    // Save to pendingPayments node
    database.ref('pendingPayments/' + payment.id).set(payment);
    
    // Add to usedUpiIds list
    database.ref('usedUpiIds').once('value').then(snapshot => {
        let usedUpiIds = snapshot.val() || [];
        usedUpiIds.push(payment.upiId);
        database.ref('usedUpiIds').set(usedUpiIds);
    });
}
...
window.onload = function() {
    ...
    // Check if there's an admin button in the UI (add one if needed)
